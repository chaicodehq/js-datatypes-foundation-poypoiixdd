...
export function createThaliDescription(thali) {
 ...
 return `${upperName} (${vegLabel}) - Items: ${itemsList} - Rs.${priceStr}`;
}
...
export function getThaliStats(thalis) {
  ...
  const vegCount = thalis.filter(thali => thali && thali.isVeg === true).length;
  const nonVegCount = thalis.filter(thali => thali && thali.isVeg === false).length;
...
export function searchThaliMenu(thalis, query) {
  ...
  return thalis.filter((thali) => {
    if (!thali) return false;

    const nameMatch =
      typeof thali.name === "string" &&
      thali.name.toLowerCase().includes(q);

    const items = Array.isArray(thali.items) ? thali.items : [];
